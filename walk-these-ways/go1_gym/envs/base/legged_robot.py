...
class LeggedRobot(BaseTask):
    ...
    def compute_reward(self):
        ...
        A = 0.005
        B = 8
        l = torch.tensor([-0.8028,-1.047,-2.696,-0.8028,-1.047,-2.696,-0.8028,-1.047,-2.696,-0.8028,-1.047,-2.696], device="cuda:0")
        h = torch.tensor([0.8028,4.188,-0.916,0.8028,4.188,-0.916,0.8028,4.188,-0.916,0.8028,4.188,-0.916], device="cuda:0")

        # l = self.dof_pos_limits[:, 0]
        # h = self.dof_pos_limits[:, 1]

        # bf = A * (1 - (torch.exp(B * (self.dof_pos - h)) + torch.exp(B * (l - self.dof_pos)))) + \
        #             A * B * self.dof_vel * (torch.exp(B * (l - self.dof_pos)) - torch.exp(B * (self.dof_pos - h)))

        # bf = -0.01 * torch.square(self.dof_pos - self.default_dof_pos)

        bf = A * (1 - (torch.exp(B * (self.dof_pos - h)) + torch.exp(B * (l - self.dof_pos))))
        
        rew_bf = torch.sum(bf, dim=1)

        self.episode_sums["vanilla"] += self.rew_buf_pos[:] * torch.exp(self.rew_buf_neg[:] / self.cfg.rewards.sigma_rew_neg)
        self.rew_buf += rew_bf
        if torch.sum(rew_bf) >= 0:
            self.rew_buf_pos += rew_bf
        elif torch.sum(rew_bf) <= 0:
            self.rew_buf_neg += rew_bf
        ...
        self.episode_sums["energy"] += torch.sum(torch.abs(self.torques) * (torch.abs(self.last_dof_pos - self.dof_pos)), dim=1)
    ...
    def _prepare_reward_function(self):
        ...
        self.episode_sums["total"] = torch.zeros(self.num_envs, dtype=torch.float, device=self.device,
                                                 requires_grad=False)
        self.episode_sums["vanilla"] = torch.zeros(self.num_envs, dtype=torch.float, device=self.device,
                                                 requires_grad=False)
    ...
    def _parse_cfg(self, cfg):
        ...
        #fix, idk why reading from config isnt working
        self.reward_scales['base_height'] = 0.05
        self.reward_scales['dof_pos'] = 0.0
        # self.reward_scales['dof_vel'] = 0.0
